    ...
    <div class="flex flex-col">
        <div class="flex flex-row-reverse items-center gap-6" >
            <div class="avatar flex flex-col w-16 justify-center items-center">
                <vs-avatar circle history :success="$page.props.user && ($page.props.user.id == comment.user.id)">
            ...
            <div class="comment-content flex-1 bg-gray-100 justify-end items-end p-4 flex flex-col">
                <div class="text-sm font-bold text-gray-700" v-html="comment.content"></div>
                ...
                <div class="inline-block relative flex flex-row-reverse gap-4 items-center justify-center shrink-0 transform scale-75">
                    ...
                    <a class="text-sm font-bold" :class="{'text-rose-600':activeReply, 'text-sky-600':!activeReply}" href="javascript:void(0)" @click.s.stop="activeReply = !activeReply">
        ...
        <div class="border-r-2 border-gray-300 rounded-br-2xl" v-if="comment.child_count">
            <ChildComments :comment="comment" :post-id="postId"/>
        </div>
...
import ChildComments from "@/Pages/Post/Show/Components/Comment/ChildComments";
